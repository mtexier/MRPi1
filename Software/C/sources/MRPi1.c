...
char s_state[100];
  ...
  if((id <= NUMBER_LEDS)&&(state <= 1))
  {
    writeCommand2parm("LED",id,state);
  }
  else
  {
    return -1;
...
/**********************************************************
 * @brief  read the groundSensor 
 * @param  sensor number (1 to 3)
 * @retval None
**********************************************************/
...
unsigned int ground = 0;
  ...
  if((sensor <= NUMBER_GROUND_SENSORS)&&(sensor > 0))
  {
    // Send the command
    writeCommand1parm("GR", sensor);
...
char str[100];
  ...
  strcpy(str, "text2wave  ");
  strcat(str,file);
  strcat(str," -o file.wav");
